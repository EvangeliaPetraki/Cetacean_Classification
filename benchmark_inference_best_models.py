...
# def find_best_run_dirs(out_root: str) -> List[str]:
#     """
#     Pick best run for each (feature_type, model, input_length) using test_macro_f1 if available,
#     otherwise val_macro_f1.
#     """
#     records = []
...
#         if not (os.path.exists(metrics_path) and os.path.exists(config_path) and os.path.exists(best_path)):
#             continue
...
#         feature = metrics.get("feature_type", cfg.get("feature"))
#         model = metrics.get("model", cfg.get("model"))
#         input_length = int(metrics.get("input_length", cfg.get("input_length", cfg.get("T", 8000))))
#         score = metrics.get("test_macro_f1", metrics.get("val_macro_f1", -1))
...
#     best_by_group = {}
#     for rec in records:
#         key = (rec["feature_type"], rec["model"], rec["input_length"])
#         if key not in best_by_group or rec["score"] > best_by_group[key]["score"]:
#             best_by_group[key] = rec

#     return [v["run_dir"] for v in best_by_group.values()]
...
def main():
    ...
    parser.add_argument("--runs_root", type=str, default="./best_models", help="Folder containing run subfolders")
    ...
    # best_run_dirs = find_best_run_dirs(args.runs_root)
    # if not best_run_dirs:
    #     raise RuntimeError("No completed runs found with metrics.json + config.json + best.pt")

    if not os.path.isdir(args.runs_root):
        raise FileNotFoundError(f"runs_root does not exist or is not a directory: {args.runs_root}")

    run_dirs = [
        os.path.join(args.runs_root, entry)
        for entry in sorted(os.listdir(args.runs_root))
        if os.path.isdir(os.path.join(args.runs_root, entry))
    ]
    ...
    for run_dir in run_dirs:
        ...
        sr = int(cfg.get("sr", args.sr))
        J = int(cfg.get("wst_J", 8))
        Q = int(cfg.get("wst_Q", 14))

        print(f"\n[Run] Benchmarking {run_id} | feature={feature} model={model_name} T={input_length}")
        ...
        model = build_model(model_name, num_classes=num_classes, in_channels=1).to(device)
        ...
        for batch_size in args.batch_sizes:
            loader, wave_batch, feat_batch = build_test_loader_and_wave_batch(
                feature=feature,
                X=X,
                y_int=y_int,
                splits=splits,
            ...
            end_to_end_mean = (
                feat_stats["mean_batch_latency_sec"] + forward_stats["mean_batch_latency_sec"]
            )
            end_to_end_sample = end_to_end_mean / batch_size
            signal_duration_sec = input_length / sr
            ...
            batch_record = {
                ...
                "feature_extraction": feat_stats,
                "model_forward": forward_stats,
                "realtime_factors": {
    ...
    for rec in results:
        for br in rec["batch_results"]:
            summary_rows.append({
                ...
                "feature_time_ms_per_sample": 1000.0 * br["feature_extraction"]["mean_sample_latency_sec"],
                "forward_time_ms_per_sample": 1000.0 * br["model_forward"]["mean_sample_latency_sec"],
